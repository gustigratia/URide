...
class SearchResultPage extends StatelessWidget {
  ...
  Widget _buildWorkshopList(List data) {
    if (data.isEmpty) {
      return const Center(
        child: Text(
          "Tidak ada hasil bengkel ditemukan",
  ...
  Widget _buildSPBUList(List data) {
    if (data.isEmpty) {
      return const Center(
        child: Text(
          "Tidak ada hasil SPBU ditemukan",
          style: TextStyle(fontSize: 16),
        ),
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final name = item['name'] ?? item['spbuName'] ?? "Tanpa Nama";
        final rating = (item['rating'] ?? "4.5").toString();
        final distance = (item['distance'] ?? "--").toString();
        final status = item['status'] ?? "Buka";
        final image = item['image_url'] ?? "assets/images/spbu.png";
        final id = item['id'];
